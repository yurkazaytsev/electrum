...
MSG_GENERATING_WAIT = _("Electrum Cash is generating your addresses, please wait...")
...
class InstallWizard(QDialog, MessageBoxMixin, BaseWizard):
    ...
    def run_and_get_wallet(self):
        ...
        def update_layout():
            ...
            self.set_layout(vbox, title=_('Electrum Cash wallet'))
            if self.pw_e:
                self.pw_e.show()
                self.pw_e.setFocus()
        ...
        if self.storage.requires_upgrade():
            self.hide()
            msg = _("The format of your wallet '%s' must be upgraded for Electrum Cash. This change will not be backward compatible"%path)
            if not self.question(msg):
                return
    ...
    def init_network(self, network):
        message = _("Electrum Cash communicates with remote servers to get "
                  "information about your transactions and addresses. The "
                  "servers all fulfil the same purpose only differing in "
                  "hardware. In most cases you simply want to let Electrum Cash "
